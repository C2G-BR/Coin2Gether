...
export class ProfilePageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe((params: ParamMap) => {
      ...
      if (username != null) {
        ...
        ).subscribe(([user, blogPosts, trades]: [User, BlogPost[], Trade[]]) => {
          this.user = user;
          this.trade_data = this.genTradeData();
          this.portfolio_data = this.genPortfolioData();
  ...
  hasPortfolioData(): Boolean {
    return (typeof this.portfolio_data != 'undefined' && this.portfolio_data.length > 0);
  }

  openDialog(): void {
    this.dialog.open(PortfolioEditDialogComponent);
  }
  ...
  genPortfolioData(): any[] {
    let table_data = new Array();
    
    if (this.user.portfolio != null) {
      if (this.user.portfolio.components != null) {
        for (let component of this.user.portfolio.components) {
          try {
            table_data.push([component.currency.acronym, component.amount]);
          } catch {}
        } 
      }
    }
