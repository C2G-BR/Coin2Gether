...
export class AuthService {
  ...
  public login(email: string, password: string) {
    ...
    return this.httpClient.post(environment.apiEndpoint + 'user/signin', body, { observe: 'response' }).pipe(retry(2), map((resp: any) => {
  ...
  public verifyToken(): void {
    ...
    this.httpClient.post(environment.apiEndpoint + 'user/verify-token', body, options).subscribe((result: any) => {
      this.isLoggedIn.next(true);
    }, error => {
